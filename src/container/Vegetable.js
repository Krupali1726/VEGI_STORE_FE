...
function Vegetable({ setIsProductDetailPage, setSelectedProductId, setIsVegetablePage }) {
    ...
    console.log("aa",typeof setSelectedProductId)
    ...
    const handleProductClick = (productId) => {
        ...
        if (setIsProductDetailPage) {
            setIsProductDetailPage(true);
        }
        if (setIsVegetablePage) {
            setIsVegetablePage(false);
        }
        // If we're in a route without props, navigate to the main page
        if (!setSelectedProductId) {
            navigate('/HomeMain');
        }
    };
                ...
                <div className="x_category-container">
                    ...
                    <Swiper
                        ...
                        breakpoints={{
                            ...
                            425: { slidesPerView: 3, spaceBetween: 30 },
                        ...
                        className="x_category-swiper ps-0 "
                    >
                        {categories.map((category, index) => (
                            <SwiperSlide key={index}>
                                ...
                                >
                                    ...
                                    <p className="x_category-name" style={{ maxWidth: '100%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                        {window.innerWidth >= 768 ?
                                            (category.categoryName.length > 20 ? `${category.categoryName.substring(0, 20)}...` : category.categoryName) :
                                            (category.categoryName.length > 10 ? `${category.categoryName.substring(0, 10)}...` : category.categoryName)
                                        }
                                    </p>
                                </div>
                            </SwiperSlide>
